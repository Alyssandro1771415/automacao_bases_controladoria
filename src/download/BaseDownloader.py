...
# read variables from .env file or use default values
...
GECKO_DRIVER_PATH = os.getenv("GECKO_DRIVER_PATH", None)

class BaseDownloader(ABC):
    def __init__(self, geckoDriver=None, download_dir=DOWNLOAD_DIR, final_dir=FINAL_DIR, log_dir=LOG_DIR) -> None:
        ...
        self.setup_logging()
        self.logger.info(f"Inicializado com download_dir: {self.download_dir}, final_dir: {self.final_dir}, log_dir: {self.log_dir}")

    def setup_directories(self):
        for path in [self.download_dir, self.final_dir, self.log_dir]:
            os.makedirs(path, exist_ok=True)

    def setup_logging(self):
        log_path = os.path.join(self.log_dir, 'downloader_log.txt')
        logging.basicConfig(
            level=logging.INFO,
    ...
    def get_driver(self, browser):
        ...
        if browser == "firefox":
            ...
            # Usa GECKO_DRIVER_PATH se definido, senão usa webdriver_manager
